    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage navigation menus ({menus.length} menus)
          </p>
        </div>
        <Link
      ...
      {menus.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          <Link
            href="/admin/menus/new"
            className="inline-flex items-center gap-2 mt-4 text-[#00D4FF] hover:underline"
          >
            <Plus className="h-4 w-4" />
            ...
            <div key={menu.id} className="bg-card border border-border rounded-lg overflow-hidden">
              ...
              {menu.items.length === 0 ? (
                <div className="p-6 text-center text-muted-foreground text-sm">
                  No menu items. <Link href={`/admin/menus/${menu.id}`} className="text-[#00D4FF] hover:underline">Add items</Link>
                </div>
              ) : (
                  ...
                  {menu.items.map((item) => (
                    <div key={item.id}>
                      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/50 transition-colors group">
                        ...
                        {!item.isActive && (
                          <span className="text-xs px-2 py-0.5 bg-yellow-500/20 text-yellow-400 rounded">Hidden</span>
                        )}
                      </div>
